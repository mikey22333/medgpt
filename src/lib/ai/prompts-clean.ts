...
export function createEnhancedMedicalPrompt(context: EnhancedMedicalPromptContext): string {
  ...
  if (mode === 'doctor') {
    ...
    prompt += "YOUR NATURAL CONVERSATION SHOULD:\n";
    prompt += "- Start by acknowledging their concern with empathy\n";
    prompt += "- Explain what might be causing their symptoms in everyday language\n";
    prompt += "- Discuss what they should be worried about vs. what's manageable\n";
    prompt += "- Give clear, actionable recommendations\n";
    prompt += "- Explain when to seek immediate medical attention\n";
    prompt += "- End with appropriate reassurance while being honest\n\n";
    prompt += "Write as one flowing conversation, like you're talking face-to-face with a patient in your office. Avoid section headers, emojis, bullet points, or formal clinical formatting.";
  } else {
